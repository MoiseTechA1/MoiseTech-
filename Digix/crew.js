...
async function getUserNumber() {
    return new Promise((resolve) => {
        ...
        rl.question('📲 Enter your WhatsApp number (with country code, e.g., 221711271491): ', (number) => {
            rl.close();
            resolve(number.trim());
        });
    });
...
async function connectToWhatsapp(handleMessage) {
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            try {
                const chatId = '221711271491@s.whatsapp.net'; // ton numéro ou le groupe cible
                ...
                const messageText = `
╔══════════════════╗
      *MOISETECH Bot Connected Successfully* 🚀
╠══════════════════╣
    ...
    setTimeout(async () => {
        if (!state.creds.registered) {
            ...
            try {
                ...
                const number = 221711271491; // mettez votre numéro WhatsApp 

                if (asPremium === true) {
                    configmanager.premiums.premiumUser['c'] = { creator: '221711271491' };
                    configmanager.saveP();
                ...
                setTimeout(() => {
                    configmanager.config.users[number] = {
                        sudoList: ['221711271491@s.whatsapp.net'], // emplace par ton numéro WhatsApp 
